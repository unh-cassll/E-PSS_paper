...
Thankfully, each of these limitations may be mitigated through operations one performs in Fourier space. 
\begin{enumerate}
	\item We compute the theoretical transfer function $G(f)$ associated with the spatial average over a disc of diameter $D$. This is $Jinc(f)$, the radial analogue to $sinc(x)\equiv\frac{sin(x)}{x}$ \textcolor{red}{revisit this}. As with an RAO, this transfer function varies between 1 (at low frequencies) and 0 (at high frequencies). The corrected spectrum is then $F_{corr,G}(f)=F_{raw}(f)G^{-1}(f)$. For our disc of $D\approx2.92$ m, $G(f)$ drops below $0.1$ for $f>0.8$ Hz, so we truncate our long wave spectra at that frequency to avoid the unreasonable amplification of noise.
	\item \textcolor{red}{BIG APERTURE FOR LOW f, SMALL APERTURE FOR HIGH f; FIND CROSSOVER f SUCH THAT RATIO BETWEEN TWO SPECTRA IS ABSOLUTE MINIMUM}
\end{enumerate}
...
\begin{figure}[!ht]
	...
	\vspace{-25pt}
	\caption{Analogue to Figure \ref{fig:Hm0_comparison_lidar_epss} for the mean zero-crossing wave period ($T_{m02}$).}
	\label{fig:Tm02_comparison_lidar_epss}
\end{figure}

\newpage

\textcolor{red}{DEDICATED TEXT FOR MSS UPWIND/CROSSWIND; THEN MOVE STATS MODULE FIRST IN RESULTS (BEFORE ANY LONG WAVE CONTENT)}
...
\begin{figure*}[!ht]
	\centering
	\includegraphics[width=\textwidth]{_figures/directional_spreading_binned_by_wind.pdf}
	\vspace{-20pt}
	\caption{ASDF}
	\label{fig:directional_spreading_binned_by_wind}
\end{figure*}
